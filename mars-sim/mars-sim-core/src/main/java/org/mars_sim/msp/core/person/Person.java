...
public class Person extends Unit implements VehicleOperator, Serializable {
	...
	public Collection<Person> getLocalGroup() throws Exception {
		...
		if (getLocationSituation().equals(Person.INSETTLEMENT)) {
			...
			if (building != null) {
				if (building
						.hasFunction(org.mars_sim.msp.core.structure.building.function.LifeSupport.NAME)) {
					org.mars_sim.msp.core.structure.building.function.LifeSupport lifeSupport = (org.mars_sim.msp.core.structure.building.function.LifeSupport) building
							.getFunction(org.mars_sim.msp.core.structure.building.function.LifeSupport.NAME);
