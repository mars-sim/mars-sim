...
extends WizardPanel {
	...
	public ConstructionProjectPanel(final CreateMissionWizard wizard) {
					...
					if (selectedSite.indexOf(" unfinished") >= 0) {
						...
						try {
							if (hasEnoughConstructionMaterials(stageInfo)) {
								...
								getWizard().setButtons(false);
								errorMessageLabel
										.setText("Not enough materials at settlement "
		...
		errorMessageLabel.setFont(errorMessageLabel.getFont().deriveFont(
				Font.BOLD));
	...
	private void populateProjectListModel() {
		...
		if (selectedSite != null) {
			if (selectedSite.equals("New Site")) {
				...
				errorMessageLabel
						.setText("Cannot start mission on site already undergoing construction.");
				// Do nothing.
			} else if (selectedSite.indexOf(" - under salvage") >= 0) {
				errorMessageLabel
						.setText("Cannot start mission on site already undergoing salvage.");
				// Do nothing.
			} else {
				Settlement settlement = getConstructionSettlement();
				if (settlement != null) {
				...
				try {
					...
					while (j.hasNext()) {
						...
						materialsList.add(new ConstructionMaterial(part
								.getName(), numRequired, numAvailable));
					}
