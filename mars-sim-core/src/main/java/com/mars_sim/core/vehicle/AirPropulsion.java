	...
	public AirPropulsion(Vehicle vehicle) {
	 ...
	 public double flyInAir(String caseText, double ascentHeight, double weight,
		 ...
		 if (ascentHeight == 0) {
			 ...
			 vAirFlow = (vMS * Math.sin(alpha2) + vPropeller * Math.cos(alpha1-alpha2)) * radiusPropeller * radPerSec; // vPropeller + vMS;
		 ...
		 double thrustForceTotal = REDUCTION_FACTOR * thrustCoefficient * 2 * airDensity * Math.PI * radiusPropellerSquare * vAirFlow * vPropeller;
...
//				 + "d: " + Math.round(distanceTravelled * 1000.0)/1000.0 + KM__
				 ...
				 + "powerDrone: " + DECIMAL3_J.format(powerThrustDrone) + TWO_WHITESPACES
