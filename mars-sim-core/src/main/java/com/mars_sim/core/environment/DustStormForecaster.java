 ...
 * of atmospheric opacity (tau) and near-surface wind speed. Outputs a hazard index in [0, 1].
 *
 * This is deliberately simple and deterministic, meant as a building block for UI/EVA guards.
 */
public final class DustStormForecaster {
    ...
    private final double tau90th;   // tau level that maps ~ to 0.9 hazard from tau alone
    ...
    /** Feed new observations. dtHours is optional (used if you want time-aware smoothing later). */
    ...
    private static double logistic01(double x) {
        ...
        // Remap so x=1 -> ~0.88 (near "0.9" interpretation).
        double k = 6.0;
