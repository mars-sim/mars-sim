...
public class TendFishTankMeta extends MetaTask implements SettlementMetaTask {
    ...
    private static class FishTaskJob extends SettlementTask {
        ...
        public FishTaskJob(SettlementMetaTask ownerTask, Settlement owner, Fishery tank, RatingScore score) {
            super(ownerTask, owner, "Tend Fish Tank", tank.getBuilding(), score);
            this.tank = tank;
        }
    ...
    public TendFishTankMeta() {
		...
		setPreferredJob(JobType.BOTANIST, JobType.ASTROBIOLOGIST, JobType.CHEMIST);
    ...
    public List<SettlementTask> getSettlementTasks(Settlement settlement) {
        ...
        for (Building building : settlement.getBuildingManager().getBuildingSet(FunctionType.FISHERY)) {
            ...
            if (result.getScore() >= 1) {
                tasks.add(new FishTaskJob(this, settlement, fishTank, result));
            }
        }
