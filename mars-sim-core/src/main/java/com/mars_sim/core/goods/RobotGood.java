...
class RobotGood extends Good {
    ...
    private static final int BASE_DEMAND = 2;
	...
	private double determineRobotDemand(GoodsManager owner, Settlement settlement) {
		...
		if (robotType == RobotType.MAKERBOT) {
			...
			baseDemand += baseDemand / (.2 + makerFactor);
		}
			...
			baseDemand += baseDemand / (.2 + repairFactor);
		}
			...
			baseDemand += baseDemand / (.2 + traderFactor);
		}
			...
			baseDemand += baseDemand / (.2 + medicFactor);
		}
