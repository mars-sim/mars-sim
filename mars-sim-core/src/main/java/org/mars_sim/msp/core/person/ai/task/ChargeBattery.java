...
/**
 * The ChargeBattery class is a task in that a robot recharges its battery.
 */
...
implements Serializable {
	...
	/** default logger. */
	private static Logger logger = Logger.getLogger(ChargeBattery.class.getName());

    /** Task name */
    private static final String NAME = Msg.getString(
            "Task.description.chargeBattery"); //$NON-NLS-1$

    /** Task phases. */
    private static final TaskPhase CHARGING = new TaskPhase(Msg.getString(
            "Task.phase.charging")); //$NON-NLS-1$

    // Static members
    private static final int NUMBER_OF_CHARGES_PER_SOL = 1;
     ...
     */
    public ChargeBattery(Person person) {
