...
//				person.getMind().getTaskManager().getNewTask();
	...
	private double waitingToEnterAirlockPhase(double time) {
		...
		if (person.isOutside()) {
			LogConsolidated.log(logger, Level.WARNING, 0, sourceName, 
			"[" + person.getLocationTag().getLocale() + "] " + person.getName() + 
			" was about to exit the airlock from inside, but was reportedly outside. "
...
//				person.registerSuit(suit);
			...
			LogConsolidated.log(logger, Level.WARNING, 4000, sourceName, 
					...
					+ " Trying to enter the airlock now.");
...
//					Settlement nearbySettlement =  ((Building) (airlock.getEntity())).getSettlement()
		...
		if (person.isOutside()) {
			LogConsolidated.log(logger, Level.WARNING, 10_000, sourceName, person.getName()
			...
			if (!goodEVASuitAvailable(airlock.getEntityInventory(), person)) {
				...
				LogConsolidated.log(logger, Level.WARNING, 10_000, sourceName, "[" + person.getLocationTag().getLocale() + "] "
...
//					person.getMind().getTaskManager().addTask(new RepairMalfunction(person));
	...
	public static EVASuit getGoodEVASuit(Inventory inv, Person p) {
		...
		for (EVASuit suit : suits) {
			...
			if (malfunction) {
				LogConsolidated.log(logger, Level.WARNING, 50_000, sourceName, "[" + p.getLocationTag().getLocale()
...
//				e.printStackTrace(System.err);
				LogConsolidated.log(logger, Level.SEVERE, 50_000, sourceName, "[" + p.getLocationTag().getLocale()
	...
	private void loadEVASuit(EVASuit suit) {
		...
		if (!(person.getContainerUnit() instanceof MarsSurface)) {
			...
			try {
				...
				LogConsolidated.log(logger,  Level.SEVERE, 10_000, sourceName, "[" + person.getLocationTag().getLocale() + "] "
