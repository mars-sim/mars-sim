...
//        }
    ...
    protected final void startTravelToNextNode() {
        ...
        legStartingTime = (MarsClock) Simulation.instance().getMasterClock()
                .getMarsClock().clone();
    }
...
//    protected abstract void performTravelPhase(Robot robot);
    ...
    public final double getTotalDistance() {
        ...
        if (navPoints.size() > 1) {
            for (int x = 1; x < navPoints.size(); x++) {
                ...
                double distance = currNav.getLocation().getDistance(
                        prevNav.getLocation());
                result += distance;
            }
        }
    ...
    public final double getTotalRemainingDistance() {
        ...
        for (int x = (index + 1); x < getNumberOfNavpoints(); x++)
            result += getNavpoint(x - 1).getLocation().getDistance(
                    getNavpoint(x).getLocation());
    ...
    public void destroy() {
        ...
        if (navPoints != null) navPoints.clear();
        navPoints = null;
        travelStatus = null;
