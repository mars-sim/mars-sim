...
public class MeteoriteImpactImpl implements MeteoriteImpact, Serializable {
	...
	/*
	 * Calculates the meteorite impact probability for the whole settlement once a sol
	 * @param BuildingManager
	 */
	// 2016-10-05 Revised calculateMeteoriteProbability() to give the incoming meteorite 
	...
	public void calculateMeteoriteProbability(BuildingManager buildingManager) {
		//System.out.println("Starting MeteoriteImpactImpl's calculateMeteoriteProbability()");
		...
		// a. average critical diameter is 0.0016 for meteorites having a spherical sphere with 8 um radius
		...
		double a_rho = AVERAGE_DENSITY - RandomUtil.getRandomDouble(.3) + RandomUtil.getRandomDouble(1.2); // in gram/cm^3
		...
		double impact_vel = IMPACT_VELOCITY * ( 1 + RandomUtil.getRandomDouble(.3) - RandomUtil.getRandomDouble(.3)); // in km/s
		...
		//System.out.println("probabilityOfImpactPerSQMPerYear : " + probabilityOfImpactPerSQMPerYear);
		...
		//System.out.println("probabilityOfImpactPerSQMPerSol : " + probabilityOfImpactPerSQMPerSol);
		
		// save it in the BuildingManager for all buildings in this settlement to apply this value
