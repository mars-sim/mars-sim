...
public final class FoodProductionUtil {
    ...
    public static List<FoodProductionProcessInfo> getAllFoodProductionProcesses() {
        return SimulationConfig
                .instance()
                .getFoodProductionConfiguration()
                .getFoodProductionProcessList();
    }
     ...
     * @throws Exception if error getting processes.
     */
    public static List<FoodProductionProcessInfo> getFoodProductionProcessesForTechLevel(
            int techLevel) {
     ...
     */
    public static List<FoodProductionProcessInfo> getFoodProductionProcessesWithGivenOutput(
            String name
            ) {
        List<FoodProductionProcessInfo> result = new ArrayList<FoodProductionProcessInfo>();
        Iterator<FoodProductionProcessInfo> i = SimulationConfig
                .instance().getFoodProductionConfiguration()
     ...
     * @return {@link List}<{@link FoodProductionProcessItem}> list of processes
     */
    public static List<FoodProductionProcessInfo> getFoodProductionProcessesWithGivenInput(
            String item
            ) {
        List<FoodProductionProcessInfo> result = new ArrayList<FoodProductionProcessInfo>();
        Iterator<FoodProductionProcessInfo> i = SimulationConfig
    ...
    /**
     * Gets foodProduction processes within the capability of a tech level and a skill level.
     * @param techLevel the tech level.
     * @param skillLevel the skill level.
     * @return list of processes.
     * @throws Exception if error getting processes.
     */
    public static List<FoodProductionProcessInfo> getFoodProductionProcessesForTechSkillLevel(
     ...
     */
    public static double getFoodProductionProcessValue(FoodProductionProcessInfo process,
            Settlement settlement) {

        double inputsValue = 0D;
        Iterator<FoodProductionProcessItem> i = process.getInputList().iterator();
        while (i.hasNext()) inputsValue += getFoodProductionProcessItemValue(i.next(), settlement, false);

        double outputsValue = 0D;
        Iterator<FoodProductionProcessItem> j = process.getOutputList().iterator();
        while (j.hasNext()) outputsValue += getFoodProductionProcessItemValue(j.next(), settlement, true);
     ...
     */
    public static double getFoodProductionProcessItemValue(FoodProductionProcessItem item,
            Settlement settlement, boolean isOutput) {
        double result = 0D;
    ...
    /**
     * Checks to see if a foodProduction process can be started at a given foodProduction building.
     ...
     */
    public static boolean canProcessBeStarted(FoodProductionProcessInfo process,
            FoodProduction kitchen) {
        boolean result = true;

        // Check to see if kitchen is full of processes.
        if (kitchen.getTotalProcessNumber() >= kitchen.getConcurrentProcesses()) result = false;
	...
	private static final boolean canProcessOutputsBeStored(FoodProductionProcessInfo process, Inventory inv)
		...
		while (j.hasNext()) {
			...
			else if (FoodProductionProcessItem.PART.equalsIgnoreCase(item.getType())) {
				Part part = (Part) ItemResource.findItemResource(item.getName());
				double mass = item.getAmount() * part.getMassPerItem();
				double capacity = inv.getGeneralCapacity();
				if (mass > capacity) result = false;
			}
			else if (FoodProductionProcessItem.EQUIPMENT.equalsIgnoreCase(item.getType())) {
