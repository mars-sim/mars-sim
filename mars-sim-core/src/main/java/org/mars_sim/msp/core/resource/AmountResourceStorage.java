...
public class AmountResourceStorage implements Serializable {
    ...
    public double getARCapacity(int resource) {
        ...
        if ((phaseStorage != null) && phaseStorage.hasAmountResourcePhaseCapacity(pt)) {
            if (
            	(phaseStorage.getAmountResourcePhaseType(pt) == null) ||
    ...
    public double getARStored(int resource) {
        ...
        if (phaseStorage != null 
        		&& ar.equals(phaseStorage.getAmountResourcePhaseType(pt))) {
...
*/
            ...
            if (hasARCapacity(resource)) {
                if (getARRemainingCapacity(resource) >= amount) {
                    ...
                    if (typeStorage != null) {
                        ...
                        double remainingTypeCapacity = typeStorage.getARTypeRemainingCapacity(
                                resource);
                        if (remainingTypeCapacity > 0D) {
                    ...
                    if (remainingAmount == 0D) {
                        ...
                        logger.severe("Amount resource " + ar + " of amount: " + amount +
                                " to store.  Amount remaining capacity: " +
                                getARRemainingCapacity(resource) + " remaining: " +
                                remainingAmount);
                    }
                }
            }

            if (!storable) throw new IllegalStateException("Amount resource: " + ar + " of amount: " +
                    amount + " could not be stored in inventory.");
        }
    }
    ...
    public void retrieveAR(int resource, double amount) {
        ...
        if (!retrievable) {
            throw new IllegalStateException("Amount resource: " + ar + " of amount: " + amount +
                    " could not be retrieved from inventory.");
        }
    }
