...
public class Person extends Unit implements VehicleOperator, Serializable {
	...
	private Map<Science, Double> scientificAchievement; // The person's
														// achievement in
														// scientific fields.
	...
	MedicalAid getAccessibleAid() {
		...
		if (location.equals(INSETTLEMENT)) {
			...
			List infirmaries = settlement.getBuildingManager().getBuildings(
					MedicalCare.NAME);
	...
	public Collection<Person> getLocalGroup() throws Exception {
		...
		if (getLocationSituation().equals(Person.INSETTLEMENT)) {
			...
			if (building != null) {
				if (building
						.hasFunction(org.mars_sim.msp.simulation.structure.building.function.LifeSupport.NAME)) {
					org.mars_sim.msp.simulation.structure.building.function.LifeSupport lifeSupport = (org.mars_sim.msp.simulation.structure.building.function.LifeSupport) building
							.getFunction(org.mars_sim.msp.simulation.structure.building.function.LifeSupport.NAME);
					localGroup = new ConcurrentLinkedQueue<Person>(lifeSupport
							.getOccupants());
				}
			}
		} else if (getLocationSituation().equals(Person.INVEHICLE)) {
			Crewable crewableVehicle = (Crewable) getVehicle();
			localGroup = new ConcurrentLinkedQueue<Person>(crewableVehicle
					.getCrew());
		}
	...
	public void setAssociatedSettlement(Settlement newSettlement) {
		if (associatedSettlement != newSettlement) {
			...
			if (oldSettlement != null)
				oldSettlement.fireUnitUpdate(
						Settlement.REMOVE_ASSOCIATED_PERSON_EVENT, this);
			if (newSettlement != null)
				newSettlement.fireUnitUpdate(
						Settlement.ADD_ASSOCIATED_PERSON_EVENT, this);
		}
	}
