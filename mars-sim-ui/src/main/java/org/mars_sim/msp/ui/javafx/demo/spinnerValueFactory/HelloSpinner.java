...
public class HelloSpinner extends Application {
    ...
    @Override public void start(Stage stage) {
        ...
        spinnerValueFactoryOptions.getSelectionModel().selectedItemProperty().addListener((o, oldValue, newValue) -> {
            switch (newValue) {
                case "Integer": {
                    spinner.setValueFactory(new SpinnerValueFactory.IntSpinnerValueFactory(5, 10));
                    break;
                }

                case "List<String>": {
                    List<String> items = FXCollections.observableArrayList("Jonathan", "Julia", "Henry");
        ...
        wrapAroundCheckBox.selectedProperty().addListener((o, oldValue, newValue) ->
                spinner.setWrapAround(newValue));
