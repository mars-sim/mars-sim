...
/**
 * The RobotTableModel maintains a list of Robot objects. By defaults
 * the source of the list is the Unit Manager.
 * It maps key attributes of the Robot into Columns.
 */
...
extends UnitTableModel {
	...
	static {
		...
		//columnNames[FATIGUE] = Msg.getString("RobotTableModel.column.fatigue"); //$NON-NLS-1$
		//columnTypes[FATIGUE] = String.class;
		//columnNames[STRESS] = Msg.getString("RobotTableModel.column.stress"); //$NON-NLS-1$
	...
	private enum ValidSourceType {
		...
		VEHICLE_ROBOTS,
		SETTLEMENT_ROBOTS,
		SETTLEMENT_ALL_ASSOCIATED_ROBOTS,
		MISSION_ROBOTS;
	}
	...
	public RobotTableModel(Crewable vehicle) {
		super(
			Msg.getString(
				"RobotTableModel.nameRobots", //$NON-NLS-1$
	...
	public RobotTableModel(Settlement settlement, boolean allAssociated) {
		super(
			(
				allAssociated ?
				Msg.getString(
					"RobotTableModel.nameAssociatedRobots", //$NON-NLS-1$
					settlement.getName()
				) :
				Msg.getString(
					"RobotTableModel.nameRobots", //$NON-NLS-1$
				...
				allAssociated ?
				"RobotTableModel.countingAssociatedRobots" : //$NON-NLS-1$
	...
	public RobotTableModel(Mission mission) {
		super(
			...
			"RobotTableModel.countingMissionMembers", //$NON-NLS-1$
			columnNames,
			columnTypes
		);
...
*/
	...
	public String getPerformanceStatus(double value) {
		String status= "N/A";
		if (value > 98 ) status = Msg.getString("RobotTableModel.column.performance.level1");
		else if (value < 99) status = Msg.getString("RobotTableModel.column.performance.level2");
		else if (value < 75) status = Msg.getString("RobotTableModel.column.performance.level3");
		else if (value < 50) status = Msg.getString("RobotTableModel.column.performance.level4");
		else if (value < 25) status = Msg.getString("RobotTableModel.column.performance.level5");
	 ...
	 */
	private static class RobotTableUpdater
	implements Runnable {
	...
	implements UnitListener {
		...
		public void unitUpdate(UnitEvent event) {
			...
			if (eventType == UnitEventType.INVENTORY_STORING_UNIT_EVENT) {
				if (event.getTarget() instanceof Robot) addUnit((Unit) event.getTarget());
			}
			else if (eventType == UnitEventType.INVENTORY_RETRIEVING_UNIT_EVENT) {
				if (event.getTarget() instanceof Robot) removeUnit((Unit) event.getTarget());
			}
		}
	}
