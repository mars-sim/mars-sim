...
public class MarqueeTicker extends JPanel {
	...
	private static final int NUM_OF_RESOURCES = 9;
	
	public static final int TIME_DELAY = 10000; // update the values every 10 real secs
...
*/	
	...
	public MarqueeTicker(SettlementWindow settlementWindow) {
		...
		marsClock = Simulation.instance().getMasterClock().getMarsClock();
	   	...
	   	// run this once just to initialize the cache values for each resource so that they are NOT having an initial value of zero
		//getResourceString();
...
*/                 
	...
	public String getOneResource(int resourceType) {
		...
		ar = AmountResource.findAmountResource(resource);
		Settlement thisSettlement = settlement;
		double newAmount = settlement.getInventory().getAmountResourceStored(ar, false);
		
		if (!thisSettlement.getResourceMapCache().containsKey(resourceType)) {
			//System.out.println(thisSettlement.getName() + " doesn't have any cache reords of resourceType " + resourceType + " yet! ");  
			// initialize the stored value of a resource
			...
			//	init++;
			//	if (init > NUM_OF_RESOURCES) 
			//		firstTime = false;
		}
		
		//System.out.println("settlement is " + settlement);
		String na = formatter.format(newAmount);
		int size_na = na.length();
		//num1 = num0 + 3 + size_na + 3;
		num1 = size_na;
		s.append(na).append(sp3);
				
		double changed = calculateChange(resourceType, newAmount);
		
		String ch = formatter.format(changed);
...
/*		
			...
			if (init > NUM_OF_RESOURCES) 
				firstTime = false;
			//System.out.println("set result to zero");
		}
*/			
		return result;
	}
