...
//            } catch (IOException e) {
//                throw new IllegalStateException("Could not find map files", e);
//            }
        } else {
            surfaceColors = mapColors;
        }
    }
    ...
    private int[] loadIndexData(String filename)  {
        ...
        for (int x = 0; x < index.length; x++) {
            try {
                ...
                throw new IllegalStateException("Could not read int", ex);
            }
        }
        try {
            indexReader.close();
        } catch (IOException ex) {
            logger.warning("Couldnt close");
        }
        try {
            indexBuff.close();
        } catch (IOException ex) {
     ...
     */
    private List<int[]> loadMapData(String filename, int[] index) {
        ...
        for (int x = 0; x < MAP_HEIGHT; x++) {
            ...
            for (int y = 0; y < colors.length; y++) {
                int red;
                try {
                    red = mapReader.readByte();
                } catch (IOException ex) {
                    throw new IllegalStateException("Could not read byte", ex);
                }
                red <<= 16;
                red &= 0x00FF0000;
                int green;
                try {
                    green = mapReader.readByte();
                } catch (IOException ex) {
                    throw new IllegalStateException("Could not read byte", ex);
                }
                green <<= 8;
     ...
     */
    private static int getRGBColor(double phi, double theta, List<int[]> surfaceColors) {
...
//                Coordinates tilecenter = new Coordinates(center.convertRectToSpherical((row - 1) * 285, (col - 1) * 285));
//			System.out.println("tilecenter  "+tilecenter.toString());
                ...
                this.createMapImage(new Coordinates(center.convertRectToSpherical((row - 1) * 285, (col - 1) * 285)), row, col, mapArray, surfaceColors);
            }
        }
        final Image createImage = displayArea.createImage(new MemoryImageSource(DISPLAY_WIDTH * 3, DISPLAY_HEIGHT * 3, mapArray, 0, DISPLAY_WIDTH * 3));
        long end = System.currentTimeMillis();
        System.out.println("createmaplarge has finished in " + (end-start) + "ms");
        return createImage;
    }

    private static void createMapImage(Coordinates center, int row, int col, int[] mapArray, List<int[]> surfaceColors) {
...
//                    int index = (DISPLAY_WIDTH - displayX) + ((DISPLAY_HEIGHT - displayY) * DISPLAY_WIDTH);
                    ...
                    if (newind < 810000) {
                        mapArray[newind] = getRGBColor(x, yCorrected, surfaceColors);
                    }
...
//                        mapArray[index] = getRGBColor(x, yCorrected, surfaceColors);
//                    }
    ...
    public void drawMap(Coordinates newCenter) {
        ...
        if ((newCenter != null) && (!newCenter.equals(currentCenter))) {
            ...
            try {
                ...
                throw new IllegalStateException("MediaTracker interrupted ", e);
