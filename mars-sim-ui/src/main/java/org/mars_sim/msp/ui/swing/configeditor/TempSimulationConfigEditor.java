...
extends JDialog {
	...
	public TempSimulationConfigEditor(Window owner, SimulationConfig config) {
		...
		super(owner, Msg.getString("TempSimulationConfigEditor.title"), ModalityType.APPLICATION_MODAL); //$NON-NLS-1$
		...
		JLabel titleLabel = new JLabel(Msg.getString("TempSimulationConfigEditor.chooseSettlements"), JLabel.CENTER); //$NON-NLS-1$
		...
		JButton addButton = new JButton(Msg.getString("TempSimulationConfigEditor.button.add")); //$NON-NLS-1$
		addButton.setToolTipText(Msg.getString("TempSimulationConfigEditor.tooltip.add")); //$NON-NLS-1$
		...
		JButton removeButton = new JButton(Msg.getString("TempSimulationConfigEditor.button.remove")); //$NON-NLS-1$
		removeButton.setToolTipText(Msg.getString("TempSimulationConfigEditor.tooltip.remove")); //$NON-NLS-1$
		...
		JButton defaultButton = new JButton(Msg.getString("TempSimulationConfigEditor.button.default")); //$NON-NLS-1$
		defaultButton.setToolTipText(Msg.getString("TempSimulationConfigEditor.tooltip.default")); //$NON-NLS-1$
		...
		createButton = new JButton(Msg.getString("TempSimulationConfigEditor.button.newSim")); //$NON-NLS-1$
		createButton.setToolTipText(Msg.getString("TempSimulationConfigEditor.tooltip.newSim")); //$NON-NLS-1$
	...
	private String determineNewSettlementName() {
		...
		while (result == null) {
			String name = Msg.getString(
				"TempSimulationConfigEditor.settlement", //$NON-NLS-1$
	...
	private String determineNewSettlementTemplate() {
		...
		else logger.log(Level.WARNING, Msg.getString("TempSimulationConfigEditor.log.settlementTemplateNotFound")); //$NON-NLS-1$
	...
	private class SettlementTableModel extends AbstractTableModel {
		...
		public String getColumnName(int columnIndex) {
			if ((columnIndex > -1) && (columnIndex < columns.length)) {
				...
				return Msg.getString("TempSimulationConfigEditor.log.invalidColumn"); //$NON-NLS-1$
			}
		}
		...
		public Object getValueAt(int row, int column) {
			Object result = Msg.getString("TempSimulationConfigEditor.log.unknown"); //$NON-NLS-1$
			...
			if ((row > -1) && (row < getRowCount())) {
				...
				if ((column > -1) && (column < getColumnCount())) {
					...
					result = Msg.getString("TempSimulationConfigEditor.log.invalidColumn"); //$NON-NLS-1$
				}
			} else {
				result = Msg.getString("TempSimulationConfigEditor.log.invalidRow"); //$NON-NLS-1$
			}
		...
		private void checkForErrors() {
			...
			while (i.hasNext()) {
				...
				if ((settlement.name == null) || (settlement.name.isEmpty())) {
					setError(Msg.getString("TempSimulationConfigEditor.error.nameMissing")); //$NON-NLS-1$
				}

				// Check if population is valid.
				if ((settlement.population == null) || (settlement.population.isEmpty())) {
					setError(Msg.getString("TempSimulationConfigEditor.error.populationMissing")); //$NON-NLS-1$
				} else {
					try {
						int popInt = Integer.parseInt(settlement.population);
						if (popInt < 0) {
							setError(Msg.getString("TempSimulationConfigEditor.error.populationTooFew")); //$NON-NLS-1$
						}
					} catch (NumberFormatException e) {
						setError(Msg.getString("TempSimulationConfigEditor.error.populationInvalid")); //$NON-NLS-1$
					}
				}
					...
					if (!cleanLatitude.endsWith(Msg.getString("direction.northShort")) && !cleanLatitude.endsWith(Msg.getString("direction.southShort"))) { //$NON-NLS-1$ //$NON-NLS-2$
						...
						try {
							...
							if ((doubleLatitude < 0) || (doubleLatitude > 90)) {
								setError(Msg.getString("TempSimulationConfigEditor.error.latitudeBeginWith")); //$NON-NLS-1$
							}
						}
						catch(NumberFormatException e) {
							setError(Msg.getString("TempSimulationConfigEditor.error.latitudeBeginWith")); //$NON-NLS-1$
					...
					if (!cleanLongitude.endsWith(Msg.getString("direction.westShort")) && !cleanLongitude.endsWith(Msg.getString("direction.eastShort"))) { //$NON-NLS-1$ //$NON-NLS-2$
						...
						try {
							...
							if ((doubleLongitude < 0) || (doubleLongitude > 180)) {
								setError(Msg.getString("TempSimulationConfigEditor.error.longitudeBeginWith")); //$NON-NLS-1$
							}
						} catch(NumberFormatException e) {
							setError(Msg.getString("TempSimulationConfigEditor.error.longitudeBeginWith")); //$NON-NLS-1$
						}
