...
public class ChatBox extends BorderPane {
	...
	public final static String INSTRUCTION = " ***** INSTRUCTIONS ***** " + System.lineSeparator()
			...
			+ "(2) Next, type in 1 to 15 (specific questions on a person/bot/settlement) or a keyword ['/k' or 'key']."
			+ System.lineSeparator() + "(3) 'bye', 'exit', 'quit', '/q', '/x', to close chat box"
			+ System.lineSeparator() + "(4) 'help', '/h' for guidance/help" + System.lineSeparator()
			+ "(5) '/y1' to reset height to 256 pixels (by default) after closing chat box. '/y2'->512 pixels, '/y3'->768 pixels, '/y4'->1024 pixels"
...
//    		textArea.appendText(YOU_PROMPT + "Farewell!" + System.lineSeparator() + "<< Disconnected from MarsNet >>" + System.lineSeparator() + System.lineSeparator());
	...
	/**
	 * Compiles the names of settlements and people and robots into the autocomplete
	 * data list
	 * 
	...
	public ObservableList<String> createAutoCompleteData() {
		...
		// autoCompleteArray = (String[])ArrayUtils.addAll(autoCompleteArray,
		// peopleRobotsArray);
...
//    		    }
			...
			else if (text.equalsIgnoreCase("key") || text.equalsIgnoreCase("/k") || text.equalsIgnoreCase("help")
					|| text.equalsIgnoreCase("/h") || text.equalsIgnoreCase("/?") || text.equalsIgnoreCase("?")) {
		...
		else if (settlementCache == null) { // better than personCache != null || robotCache != null since it can
...
//	    	}
	 ...
	 */
	public boolean isQuitting(String text) {
		if (text.equalsIgnoreCase("quit") || text.equalsIgnoreCase("/quit") || text.equalsIgnoreCase("\\quit")
				|| text.equalsIgnoreCase("\\q") || text.equalsIgnoreCase("/q")

				|| text.equalsIgnoreCase("exit") || text.equalsIgnoreCase("/exit") || text.equalsIgnoreCase("\\exit")
				|| text.equalsIgnoreCase("\\x") || text.equalsIgnoreCase("/x")

				|| text.equalsIgnoreCase("bye") || text.equalsIgnoreCase("/bye") || text.equalsIgnoreCase("\\bye")) {
			return true;
...
//			System.out.println("/p is submitted");
			responseText = SYSTEM_PROMPT + "Below is a list of proposals for your review :"	
				+ System.lineSeparator() + "1. Safety and Health Measures"
					+ System.lineSeparator() + "2. Manufacturing Priority"
						+ System.lineSeparator() + "3. Food Allocation Plan";
		}
				...
				if (!available) {
					...
					if (!personList.isEmpty()) {
						...
						if (person.getPhysicalCondition().isDead()) {
							...
							if (rand == 0)
								responseText = SYSTEM_PROMPT + "I'm sorry. " + text
										+ " has passed away and is buried at " + person.getBuriedSettlement().getName();
							else
								responseText = SYSTEM_PROMPT + "Perhaps you don't know that " + text
										+ " is dead and is buried at " + person.getBuriedSettlement().getName();
						} else
							unit = person;
					}
	...
	public void hitEnter() {
		...
		if (text != "" && text != null && !text.trim().isEmpty()) {
			...
			if (personCache == null && robotCache == null && settlementCache == null) {
				...
				// if both personCache and robotCache are set to null, then quit the
				// askQuestion() stage and go back to parseText() stage
				askQuestion(text);
			}
